...
export default function ProjectProfileFullForm({
  ...
  return (
    <>
      <form className="space-y-6 sm:space-y-10 -mt-16 sm:-mt-24">
        {FORM_SECTIONS.map((section, idx) => (
          <div key={idx} className="bg-white rounded-lg shadow p-4 sm:p-6">
            {(section.title || idx === 0) && (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3 sm:gap-4 relative z-10">
                {section.title && (
                ...
                {idx === 0 && (
                  <div className="flex flex-wrap gap-2 sm:gap-3 order-2 sm:order-1">
                    {/* Nút Xem trước - Cải thiện style */}
                    <button
                      type="button"
                      className="px-4 py-2 sm:px-6 sm:py-3 bg-yellow-600 text-white font-medium rounded-lg shadow-md hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105 z-20 flex items-center gap-1 sm:gap-2 text-sm sm:text-base flex-1 sm:flex-none"
                      onClick={handleViewPdf}
                    >
                      <svg
                    ...
                    <PDFDownloadLink
                      document={<MyDocument data={form} />}
                      fileName="ho-so-du-an.pdf"
                      className="px-4 py-2 sm:px-6 sm:py-3 bg-green-600 text-white font-medium rounded-lg shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105 no-underline z-20 flex items-center gap-1 sm:gap-2 text-sm sm:text-base flex-1 sm:flex-none"
                      onClick={handleDownload}
                    >
                      {({ loading }) => (
                        <>
                    ...
                    <button
                      type="button"
                      className="px-4 py-2 sm:px-6 sm:py-3 bg-blue-600 text-white font-medium rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105 z-20 flex items-center gap-1 sm:gap-2 text-sm sm:text-base flex-1 sm:flex-none"
                      onClick={handlePublishClick}
                    >
                      <svg
                        className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0"
                    ...
                    <button
                      type="button"
                      className="px-4 py-2 sm:px-6 sm:py-3 bg-gray-600 text-white font-medium rounded-lg shadow-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105 z-20 flex items-center gap-1 sm:gap-2 text-sm sm:text-base flex-1 sm:flex-none"
                      onClick={handleSaveClick}
                    >
                      <svg
                        className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0"
            ...
            {section.subtitle && (
              <h3 className="text-sm sm:text-base font-semibold mb-2 sm:mb-4 text-blue-700">
                {section.subtitle}
              </h3>
            )}
              ...
              {section.fields.map((field) => (
                ...
                >
                  ...
                  {field.type === "input" && (
                    <input
                      type="text"
                      className="border rounded p-2 sm:p-3 bg-gray-50 text-sm"
                      value={form[field.key]}
                      placeholder={field.placeholder}
                      onChange={(e) => handleChange(field.key, e.target.value)}
                    />
                  ...
                  {field.type === "textarea" && (
                    <textarea
                      className={
                        field.key === "teamInfo"
                          ? "border rounded p-2 sm:p-3 bg-gray-50 min-h-[120px] w-full md:w-[600px] text-sm"
                          : "border rounded p-2 sm:p-3 bg-gray-50 min-h-[48px] text-sm"
                      }
                      value={form[field.key]}
                      placeholder={field.placeholder}
                      onChange={(e) => handleChange(field.key, e.target.value)}
                  ...
                  {field.type === "image" && (
                    <div className="space-y-2">
                      ...
                      <label
                        htmlFor={`file-${field.key}`}
                        className={`${styles["file-picker-btn"]} text-sm`}
                      >
                        {uploading[field.key] ? "Đang tải..." : "Chọn ảnh"}
                      </label>
                      {form[field.key] && (
                        <div className="mt-2">
                          <div className={`${styles.thumb} max-w-full`}>
                            <img
                              src={form[field.key]}
                              alt="preview"
                  ...
                  {field.type === "images" && (
                    <div className="space-y-2">
                      ...
                      {Array.isArray(form[field.key]) &&
                        form[field.key].length > 0 && (
                          <div
                            className={`${styles.thumbnails} grid grid-cols-2 sm:grid-cols-3 gap-2`}
                          >
                            {form[field.key].map((src, i) => (
                              <div
                                key={i}
                                className={`${styles.thumb} relative w-full aspect-square`}
                              >
                                <img
                                  src={src}
                                  alt={`preview-${i}`}
                                ...
                                <button
                                  type="button"
                                  className={`${styles.removeBtn} text-xs`}
                                  onClick={() => removeImageAt(field.key, i)}
                                >
                                  ×
                                </button>
      ...
      {/* Modal cho Xem trước PDF - Cải thiện modal để chuyên nghiệp hơn */}
      {showPdfModal && (
        <div className="fixed inset-0 z-50 flex justify-center items-stretch bg-black bg-opacity-60 backdrop-blur-sm p-2 sm:p-0">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-7xl h-[95vh] flex flex-col border border-gray-200">
            <div className="p-3 sm:p-6 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-blue-50 to-indigo-50 rounded-t-xl">
