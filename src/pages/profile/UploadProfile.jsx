...
function AutoResizeTextarea({
  ...
  const resize = () => {
    ...
    el.style.height =
      (maxHeight ? Math.min(newHeight, maxHeight) : newHeight) + "px";
        ...
        <nav className="space-y-1">
          {sections.map((s) => (
            <a
              key={s.id}
              href={`#${s.id}`}
              className="block px-2 py-1.5 rounded text-sm text-gray-700 hover:bg-gray-50"
            >
              {s.label}
            </a>
          ))}
...
export default function UploadProfile() {
  ...
  const {
    ...
    error: postsError,
  } = usePosts(projectId, { pageSize: 20 });
  ...
  async function loadProject() {
    ...
    try {
      ...
      setProjectData((prev) => ({
        ...
        team_image_url:
          data.team_image_url || data.team_image?.url || prev.team_image_url,
        ...
        distribution_channel:
          data.distribution_channel || prev.distribution_channel,
        ...
        deployment_location:
          data.deployment_location || prev.deployment_location,
      }));
      ...
      if (memRes.ok) {
        ...
        setMembers(
          mems.map((m) => ({
            id: m.id,
            name: m.name || (m.user_id ? `User ${m.user_id}` : ""),
            position: m.role_in_project || "",
            avatar: m.avatar_url || "",
          }))
        );
      }
      ...
      if (ms > 2000) {
        showToast(
          "Máy chủ phản hồi chậm khi tải project (" + ms + "ms)",
          "warning",
          2500
        );
      }
  ...
  const handleAddMember = async () => {
    if (!projectId) {
      showToast("Project ID không xác định", "error");
      return;
    }
    ...
    if (!newMember.position || !newMember.position.trim()) {
      showToast("Vui lòng nhập vị trí", "warning");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showToast(
          "Thêm thành viên thất bại: " + txt.substring(0, 150),
          "error"
        );
        return;
      }
  ...
  const saveProject = async () => {
    ...
    if (!ALLOWED_STAGES.includes(projectData.stage)) {
      showToast(
        "⚠️ Stage không hợp lệ. Vui lòng chọn giai đoạn hợp lệ.",
        "warning"
      );
      return;
    }
    ...
    if (projectData.website_url && projectData.website_url.trim()) {
      ...
      if (validationResult === false) {
        showToast(
          "⚠️ Website URL không hợp lệ. Vui lòng nhập URL đúng định dạng.",
          "warning"
        );
        return;
      }
    ...
    try {
      ...
      if (projectFiles.logoFile) {
        try {
          ...
          showToast(
            `⚠️ Upload logo thất bại: ${err.message}. Vui lòng thử lại.`,
            "warning",
            5000
          );
        } finally {
      ...
      if (projectFiles.bannerFile) {
        try {
          ...
          showToast(
            `⚠️ Upload banner thất bại: ${err.message}. Vui lòng thử lại.`,
            "warning",
            5000
          );
        } finally {
      ...
      if (
        typeof team_image_url === "string" &&
        team_image_url.startsWith("data:")
      ) {
      ...
      const optionalFields = {
        ...
        member_count:
          typeof projectData.member_count === "string"
            ? parseInt(projectData.member_count, 10)
            : typeof projectData.member_count === "number"
        ...
        try {
          ...
          if (errorData) {
            if (errorData.backend_response) {
              ...
              errorMessage =
                errorData.message || JSON.stringify(errorData.backend_response);
              ...
              errorMessage = `Không kết nối được tới backend (${
                errorData.message || "unknown error"
              }). URL: ${errorData.request_url}`;
            } else {
      ...
      if (ms > 2500) {
        showToast(
          "Máy chủ xử lý lưu chậm (" + ms + "ms). Ảnh lớn có thể làm chậm.",
          "warning",
          3000
        );
      }
      ...
      const msg =
        err?.name === "AbortError"
          ...
          : "Lỗi khi lưu project: " + err.message;
      showToast(msg, "error");
    } finally {
      setSaving(false);
    }
  ...
  const publishProject = async () => {
    ...
    try {
      ...
      const res = await fetchWithTimeout(
        `${API_BASE}/projects/${projectId}/publish`,
        {
          method: "POST",
          headers: authHeaders(token),
          timeout: 10000,
      ...
      const msg =
        err?.name === "AbortError"
          ? "Máy chủ phản hồi quá lâu (timeout) khi publish."
          : "Lỗi khi đăng tải project";
      showToast(msg, "error");
    } finally {
      setSaving(false);
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
          ...
          <div className="lg:col-span-9">
            <div id="overview" className="bg-white rounded-lg shadow p-6">
              ...
              <div className="relative h-48 bg-gray-200 rounded-lg overflow-hidden mb-5 group">
                ...
                ) : projectData.team_image_url &&
                  projectData.team_image_url.length > 0 ? (
                  <>
                    <img
                    ...
                    {bannerProgress > 0 && bannerProgress < 100 && (
                      <div className="absolute bottom-0 left-0 right-0 bg-black/40 p-2">
                        ...
                        <div className="text-xs text-white mt-1">
                          Đang tải banner: {bannerProgress}%
                        </div>
                      </div>
                    )}
                  </>
                    ...
                    <div className="text-center">
                      ...
                      {projectData.team_image_url && (
                        <p className="text-xs mt-2 text-gray-400">
                          (Kích thước: {projectData.team_image_url.length}{" "}
                          bytes)
                        </p>
                      )}
                      <p className="text-xs mt-1 text-gray-400">
                        Khuyến nghị: 1600×600px, &lt; 1.5MB.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleProjectChange("tagline", e.target.value)
                      }
                      placeholder="VD: Công cụ quản lý dự án hỗ trợ AI"
                      className="w-full p-2 border rounded-lg"
                      maxLength="150"
                    />
                    <p className="text-[11px] text-gray-500 mt-1">
                      {projectData.tagline.length}/150
                    </p>
                  </div>
                  <div className="mb-4">
                    <h3 className="font-semibold mb-2">
                      Giới thiệu dự án <span className="text-red-500">*</span>
                    </h3>
                    <AutoResizeTextarea
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleProjectChange("industry", e.target.value)
                      }
                      placeholder="VD: SaaS, Fintech, Edtech, Healthtech"
                      className="w-full p-2 border rounded-lg"
                    />
                  </div>
                </div>
                  ...
                  <div className="bg-gray-50 p-5 rounded-lg space-y-3.5">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleProjectChange("website_url", e.target.value)
                        }
                        placeholder="https://example.vn"
                        className="w-full p-2 border rounded"
                      />
                      <p className="text-[11px] text-gray-500 mt-1">
                        VD: https://example.vn (tự động thêm https:// nếu thiếu)
                      </p>
                    </div>
                    <div>
                      <h3 className="font-semibold mb-2">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleProjectChange("stage", e.target.value)
                        }
                        className="w-full p-2 border rounded"
                        required
                        aria-required="true"
                      >
                        ...
                        <option value="nghien-cuu-thi-truong">
                          (nghiên cứu thị trường)
                        </option>
                        <option value="hoan-thien-san-pham">
                          (hoàn thiện sản phẩm)
                        </option>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
                <div>
                  ...
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleProjectChange("pain_point", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={3}
                ...
                <div>
                  ...
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleProjectChange("solution", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={3}
                ...
                <div>
                  ...
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleProjectChange("product", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={3}
                ...
                <div>
                  ...
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleProjectChange("customer_segment", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={3}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleProjectChange("market_size", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleProjectChange("market_area", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
                <div>
                  ...
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleProjectChange("business_model", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={3}
                ...
                <div>
                  <h3 className="font-semibold mb-2">
                    Phương thức tạo doanh thu
                  </h3>
                  <AutoResizeTextarea
                    value={projectData.revenue_method}
                    onChange={(e) =>
                      handleProjectChange("revenue_method", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={3}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleProjectChange("revenue_goal", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleProjectChange("cost_estimate", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleProjectChange("capital_source", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
            ...
            <div id="partners" className="border-t pt-6 mt-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <h3 className="font-semibold mb-2">
                    Đối tác/Investor hiện tại
                  </h3>
                  <AutoResizeTextarea
                    value={projectData.partners}
                    onChange={(e) =>
                      handleProjectChange("partners", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={3}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleProjectChange("deployment_location", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
            ...
            <div id="posts" className="border-t pt-6 mt-6">
              ...
              {postsError && (
                <div className="text-sm text-red-600 mb-3">
                  Lỗi tải bài viết: {postsError.message}
                </div>
              )}
              {postsLoading ? (
                    ...
                    posts.map((p) => (
                      <div key={p.id} className="p-4 border rounded-lg">
                        ...
                        <div className="text-gray-900">
                          {p.title || p.body || "(không có nội dung)"}
                        </div>
                      </div>
                    ))
                  ) : (
                  ...
                  {postsHasMore && (
                    <button
                      ...
                      className={`bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded ${
                        postsLoadingMore ? "opacity-60 cursor-wait" : ""
                      }`}
                    >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div>
                  ...
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleProjectChange("member_skills", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={4}
                ...
                <div>
                  ...
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleProjectChange("resources", e.target.value)
                    }
                    className="w-full p-2 border rounded-lg"
                    minRows={4}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur border-t border-gray-200 px-3 py-2 z-40">
        <div className="max-w-5xl mx-auto flex items-center justify-end gap-2.5">
          <button
            onClick={() => saveProject()}
            disabled={saving}
            className={`bg-gray-700 hover:bg-gray-800 text-white text-sm font-semibold px-4 py-1.5 rounded-md ${
          ...
          {(() => {
            ...
            const isValidRequired =
              projectData.name?.trim()?.length >= 3 &&
              projectData.description?.trim()?.length > 0 &&
              ALLOWED_STAGES.includes(projectData.stage);
            return (
              <button
                ...
                className={`bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 py-1.5 rounded-md ${
                  saving || !projectId || !isValidRequired
      ...
      {toast.visible && (
        <div className="fixed right-4 bottom-6 z-50">
          <div
            className={`flex items-start gap-3 px-4 py-2 rounded shadow text-white ${
              toast.type === "error"
                ? "bg-red-600"
                : toast.type === "success"
                ? "bg-green-600"
